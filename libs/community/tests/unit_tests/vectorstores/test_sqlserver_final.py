...
def test_can_connect_with_entra_id():
    ...
    with patch(
        ...
        json_string = json.dumps(parsed_url, indent=4)
...
#     store._embedding_store.__table__ = mock_table
#
#     # Patch the Session and create method
#     with patch('langchain_community.vectorstores.sqlserver.Session', wraps=MagicMock()) as mock_session, \
#         patch(
#             f"langchain_community.vectorstores.sqlserver.SQLServer_VectorStore._create_table_if_not_exists",
#             wraps=SQLServer_VectorStore._create_table_if_not_exists,
#             ), \
#             patch('langchain_community.vectorstores.sqlserver.Session.get_bind', wraps=MagicMock()) as mock_get_bind, \
#             patch.object(mock_session, 'commit') as mock_commit, \
#             patch.object(mock_table, 'create') as mock_create:
#
#         mock_get_bind.return_value = 'mock_bind'
#         store._create_table_if_not_exists(store)
#         print(mock_session)
#         print(mock_get_bind)
#
#         mock_create.assert_called_once_with('mock_bind', checkfirst=True)
#         mock_commit.assert_called_once()
#
#
# def test_create_table_if_not_exists_programming_error():
#     # Arrange
#     store, mocks = generalized_mock_factory()
#     mock_session = MagicMock()
#     mock_table = MagicMock()
#     store._embedding_store.__table__ = mock_table
#     store._bind = MagicMock()
#     mock_table.create.side_effect = ProgrammingError("error", "params", "orig")
#
#     with patch(
...
#         mock_session.assert_called_once_with(store._bind)
#         mock_table.create.assert_called_once_with(
#             mock_session.get_bind(), checkfirst=True
#         )
...
def test_similarity_search():
    ...
    with patch(
        ...
        store.embedding_function.embed_query.return_value = mock_responses["hi"]
        ...
        store.embedding_function.embed_query.return_value = mock_responses["hello"]
...
def test_create_filter_clause():
    ...
    with (
        ...
        filter_value = {"id": {"$eq": [1, 5, 2, 9]}, "location": {"$eq": ["pond", "market"]}}
